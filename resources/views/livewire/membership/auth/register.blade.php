        ...
        <div class="px-4 py-8 bg-white shadow sm:rounded-lg sm:px-10">
            <form wire:submit.prevent="register" noValidate=true>
                ...
                <div class="mt-6">
                    ...
                    <div class="mt-1 rounded-md shadow-sm">
                        <select wire:model.lazy="membershipPlan" id="membership_plan" class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md placeholder-gray-400 focus:outline-none focus:ring-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5 @error('membershipPlan') border-red-300 text-red-900 placeholder-red-300 focus:border-red-300 focus:ring-red @enderror">
                            <option selected disabled value="">Select Membership Plan</option>
                            @foreach($membershipPlanList as $key => $item)
                                <option value="{{ $item->id }}">{{ $item->name }} </option>
                            @endforeach
                        </select>
                    </div>

                    @error('membershipPlan')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
